"""Build candidate lists for music-to-text cycle-consistency pairs."""
...
from dataset.paths import CAPTION_DATA, DATA_ROOT, RESOURCES
        ...
        for _, row in df.iterrows():
            ...
            for method in methods:
                ...
                if ytid not in caption_dict[method]:
                    continue
                candidates.append(
                    {"caption": caption_dict[method][ytid], "score": row[method], "method": method}
                )
            if not candidates:
                continue
...
def main():
    similarity_path = RESOURCES / "laion-disco_mulan_m2t.json"
    caption_paths_dict = {
    ...
    output_path = DATA_ROOT / "base/m2t/ld-mnt-muqmulan.jsonl"
    build_dpo_dataset(similarity_path, caption_paths_dict, CAPTION_DATA / "laion-disco-10s-ytid.csv", output_path)
