...
if __name__ == "__main__":
    datasets_dir = PROJECT_ROOT / "generate_dataset/CPPair-t2m/datasets"
        
    for f in os.listdir(datasets_dir):
        if f.endswith(".jsonl"): 
            input_jsonl = os.path.join(datasets_dir, f)
            output_jsonl = str(PROJECT_ROOT / f"generate_dataset/CPPair-t2m/threshold/CPpair-{f}")

            THRESH_SIMILAR = 0.005
            THRESH_NEGATIVE = 0.4
