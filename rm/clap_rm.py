...
class CLAPReward(nn.Module):
    def __init__(self, ft_audio_ratio=0.2):
        super().__init__()
        ...
        if hasattr(audio_branch, 'layers'):
            ...
            num_ft_blocks = int(num_blocks * ft_audio_ratio)
            ...
            # Unexpected structure: unfreeze entire audio_branch as fallback.
            print("Warning: 'layers' attribute not found. Unfreezing entire audio_branch.")
            for p in audio_branch.parameters():
                p.requires_grad = True
    ...
    def forward(self, audio, text):
        ...
        # 2. Match device.
        device = next(self.reward_head.parameters()).device
        a = a.to(device)
        t = t.to(device)
...
def main():
    ...
    model = CLAPReward(
        ft_audio_ratio=0  # Fraction of layers to fine-tune.
    ).to(device)
